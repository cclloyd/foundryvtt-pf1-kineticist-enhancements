...
import type {BlastConfig, BlastConfigs, BlastDamagePart, BlastDamageParts} from "#ke/types/blasts";
...
export default class BlastFactory {
    ...
    formInfusion?: FormInfusionConfig;
    substanceInfusion?: SubstanceInfusionConfig;
    metakinesis?: MetakinesisConfig[];
    utilityTalents?: UtilityTalentConfig[];
    ...
    async parseInput() {
        const customFormInfusions = getSetting('customFormInfusions') as FormInfusionConfigs;
        const customSubstanceInfusions = getSetting( 'customSubstanceInfusions') as SubstanceInfusionConfigs;

        // Fetch infusions
        let formInfusion: FormInfusionConfig;
    ...
    async initBlastData() {
        ...
        const customBlasts = getSetting('customBlasts') as BlastConfigs;
        ...
        let blastConfig: BlastConfig = simpleBlastsWith3pp[this.formData['blast']];
    ...
    async setBaseDamage() {
        // Base simple blast
        let BASE = ['(ceil(@classes.kineticist.level / 2))d6', 'Simple'] as BlastDamagePart;
        // Elemental Overflow
        let EO = [
    ...
    async applyBlastTransforms() {
        ...
        if (this.blastConfig!.transform && typeof this.blastConfig!.transform === 'function') {
    ...
    async applyUtilityTalents() {
        const customUtilityTalents = getSetting('customUtilityTalents') as UtilityTalentConfigs;
    ...
    async applyMetakinesis() {
        const customMetakinesis = getSetting('customMetakinesis') as MetakinesisConfigs;
